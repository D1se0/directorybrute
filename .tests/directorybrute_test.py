...
def check_url(queue, base_url, progress_bar, hidden_statuses, hidden_sizes, extensions, hidden_directories, output_file=None):
    global stop_threads
    collected_results = []
    ...
    while not queue.empty() and not stop_threads:
        ...
        try:
            ...
            if status_code not in hidden_statuses and content_length not in hidden_sizes:
                ...
                collected_results.append((base_url.replace('BRUTE', word), status_code, content_length))
                if output_file:
        ...
        for ext in extensions:
            ...
            try:
                ...
                if status_code not in hidden_statuses and content_length not in hidden_sizes:
                    ...
                    print(f"\r{Fore.GREEN}[+]{Style.RESET_ALL} {result.ljust(os.get_terminal_size().columns - 1)}", end="")
                    collected_results.append((base_url.replace('BRUTE', f'{word}.{ext}'), status_code, content_length))
                    if output_file:
                        with open(output_file, 'a') as f:
                            f.write(f"{result}\n")
...
def main():
    ...
    for _ in range(args.threads):
        t = threading.Thread(target=check_url, args=(task_queue, base_url, progress_bar, hidden_statuses, hidden_sizes, extensions, args.hd, args.file))
        threads.append(t)
        t.start()
    ...
    if args.file:
        # Capturar los resultados para escribir en el archivo
        collected_results = check_url(task_queue, base_url, progress_bar, hidden_statuses, hidden_sizes, extensions, args.hd, args.file)
        ...
        with open(args.file, 'a') as f:
            ...
            for result in collected_results:
                f.write(f"{result[0]} [{result[1]}] - {result[2]} bytes\n")
